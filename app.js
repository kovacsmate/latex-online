...
LatexOnline.create(__dirname + '/public/downloads/', __dirname + '/public/storage/')
    .then(onInitialized)
...
function sendError(res, userError) {
    ...
    var json = JSON.stringify({
        ...
        errorText: userError || 'Internal Server Error'
    });
    res.send(json);
}
...
async function handleResult(res, preparation, force, downloadName) {
    ...
    if (compilation.userError) {
        ...
        res.download(compilation.outputPath());
...
app.get('/compile', async (req, res) => {
    ...
    if (isBrowser && (!trackId || !pendingTrackIds.has(trackId))) {
        ...
        var redirectURL = '/compile?' + search;
